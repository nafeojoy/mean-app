...
export class TopSlideLoaderComponent {
    ...
    showModal(item) {
        ...
        this.images = item.preview_images;
    ...
    getPreviewData(pagenum) {
        if (this.images.length > 0) {
            this.image = this.images[pagenum].image[Object.keys(this.images[pagenum].image)[0]];
            console.log(this.image)
        }
        this.bookImageCount = this.image.length;
