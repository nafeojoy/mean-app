...
function defaultImageConfig(id, model, imageIndex, pageNum, imageSizes) {
  var _id = id;
  var defaultImageConfig = {
    ...
    field: _id,
    path: config.IMAGE_UPLOAD_PATH + model + '/' + imageIndex + '/' + _id + '/previews/' + pageNum + '/image/',
    pathResize: config.IMAGE_UPLOAD_PATH + model + '/' + imageIndex + '/' + _id + '/previews/' + pageNum + '/',
    directory: 'image'
  };
...
function formEndPromise(fields, files, form, model, import_id, imageIndex, pageNum, imageSizes) {
  ...
  var _id = import_id;
  imageConfig = new defaultImageConfig(_id, model, imageIndex, pageNum, imageSizes);
      ...
      if (err) {
        ...
        for (var key in imageSizes) {
          if (imageSizes.hasOwnProperty(key)) {
            ...
            imagePaths[dimension] = "/images/" + model + "/" + imageIndex + "/" + _id + "/previews/" + pageNum + "/" + dimension + "/" + file_name;
          }
        }
        resolve(imagePaths)
      }
...
export function previewUploader(req, imageDirectory, import_id, imageIndex, pageNum, imageSizes) {
  return new Promise((resolve, reject) => {
    ...
    form.on('end', function (fields, files) {
      formEndPromise(fields, files, this, imageDirectory, import_id, imageIndex, pageNum, imageSizes).then(response => {
        resolve(response);
      })
    });
  })
...
function resizeimg(thumb, dimensions) {
  ...
  thumb = thumb + file_name;
  gm(fullpath)
    .draw([config.BACK_IMAGE_WATERMARK_PATH_STRING])
