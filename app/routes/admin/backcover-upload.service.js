...
function defaultImageConfig(id, model, imageIndex, imageSizes) {
  ...
  var defaultImageConfig = {
    ...
    field: _id,
    path: config.IMAGE_UPLOAD_PATH + model + '/' + imageIndex + '/' + _id + '/backcover' + '/image/',
    pathResize: config.IMAGE_UPLOAD_PATH + model + '/' + imageIndex + '/' + _id + '/backcover/',
    directory: 'image'
  };
      ...
      if (err) {
        ...
        for (var key in imageSizes) {
          if (imageSizes.hasOwnProperty(key)) {
            ...
            imagePaths[dimension] = "/images/" + model + "/" + imageIndex + "/" + _id + "/backcover/" + dimension + "/" + file_name;
          }
        }
        resolve(imagePaths)
      }
...
export function backCoverUploader(req, imageDirectory, import_id, imageIndex, imageSizes) {
