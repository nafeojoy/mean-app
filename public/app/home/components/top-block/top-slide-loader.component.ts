...
@Component({
    selector: 'top-slide-loader',
    templateUrl: './top-slide-loader.html',
    encapsulation: ViewEncapsulation.None
})

export class TopSlideLoaderComponent {
    ...
    private _urlParams = new BehaviorSubject<any[]>([]);
    ...
    //  constructor(private homeService: HomeService, private cartService: CartService, private _cookieService: CookieService, private pubSubService: PubSubService) { 
    constructor(private homeService: HomeService, private pubSubService: PubSubService, private cartService: CartService) {
        this.Math = Math;
    ...
    @Input()
    set urlParams(value) {
        this._urlParams.next(value);
    }

    get urlParams() {
        return this._urlParams.getValue();
    }
    ...
    addToCart(item, index) {
        $(".float-cart").css("right", "10px");
        $(".float-cart").css("background-color", "green");
